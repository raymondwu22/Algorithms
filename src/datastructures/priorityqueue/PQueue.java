...
public class PQueue <T extends Comparable<T>> {
    ...
    // dynamic list to track elements within heap
    ...
    // Construct a initially empty priority queue
    public PQueue() {
        this(1);
    }
    ...
    // returns true/false depending if the priority queue is empty
    ...
    // priority in this PQueue. If the PQueue is empty
    // null is returned
    public T peek() {
    ...
    // adds an element to the PQueue, the
    // element must not be null, O(log(n))
    public void add(T elem) {
        if (elem == null) throw new IllegalArgumentException();
        // check if we need to expand the capacity of our heap
        ...
        // new item added to the end of the list, and item will
        // need to swim upwards to its appropriate index
        swim(heapSize);
        heapSize++;
    }
    ...
    private boolean less(int i, int j) {
        T node1 = heap.get(i);
        T node2 = heap.get(j);
        // utilizes the fact that the two nodes are comparable
        // thus we are able to invoke the compareTo method
    ...
    private void swim(int k) {
        ...
        while (k > 0 && less(k, parent)) {
            swap(parent, k);
            k = parent;
            // Grab the index of the net parent node WRT to k
            parent = (k - 1) / 2;
        }
    }
    ...
    // removes a particular element in teh heap, O(log(n))
    ...
    private T removeAt(int i) {
        ...
        if (heap.get(i).equals(removed_data)) {
            swim(i);
        }
        return removed_data;
    }
    ...
    // This method is just for testing purposes to m ake
    // sure theheap invariant is still being maintained
    // call this method with k=0 to start at the root
    public boolean isMinHeap(int k) {
        // if we are outside the bounsd of the heap return true
    ...
    // Add a node value to tis index to the map
    ...
    // eXchange the index of two nodes internall within the map
