...
class Entry<K, V> {
    ...
    // we are note overriding the Object equals method
    // no cashing is required with this method.
...
public class HashTableSeparateChaining <K,V> implements Iterable<K> {
    ...
    // Strip the negative sign and place the hash value in the domain [0, capacity)
    ...
    // Get a key's value from the map and returns the value
    public V get(K key) {
    ...
    private V bucketInsertEntry(int bucketIndex, Entry<K,V> entry) {
        ...
        if (existentEntry == null) {
            ...
            return null; // use null to indicate that htere was no prev entry
        } else {
